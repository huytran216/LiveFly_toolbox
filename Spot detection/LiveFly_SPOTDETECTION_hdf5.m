...
mov_folder='C:\Users\bvhutr\Dropbox (UMR3664)\backup\Memory_analysis\Mounia data\RAW\mov2';                       %indicate the full path movies file
main_mov='embryo2.h5';                            %title of ensemble movie (with extension)
...
if strcmp(questdlg('Choose a configuration file?'),'Yes')
    ...
    shift_left=950;        % Length of the cat portion on the left
    shift_right=950;       % Length of the cat portion on the right
    dt=24.7;                   % Time step in seconds that you read from the metafile
    A_pole=1;               % Anterior pole position 1=left
    channel=0;              % Channel for Nuclei image (0 or 1)
    x_resolution=0.198;     % Pixel size (micron/pixels) on XY axis (optional)
    z_resolution=0.5;       % Pixel size (micron/pixels) on Z axis

    % IMPORTANT PARAMETERES FOR SPOT DETECTION %% 
    th1=53;                 % Minimal threshold for spot detection, set to 0 for debug mode
    th2=40;                 % Minimal threshold for median filter, set to 0 for debug mode
    th=[th1 th2];
...
Itmp = h5read(ms2_mov,'/t0/channel0')/2^(nbit-8);
z_max = size(Itmp,3);
...
for it=it_start:it_end
    ...
    [spot,Ispot,raw_spot]=find_ms2_spots_1spot_hdf5(ms2_mov,channel,it,nuc,th,z_max,voxels_min,voxels_max,fact_r,window_around_spot,averaging_radius);
